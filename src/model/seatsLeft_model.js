...
const seatsModel = {
    ...
    create: ({ site, status }) => {
        return new Promise((resolve, reject) => {
            db.query(
                `INSERT INTO seats_left (id, site, status) VALUES ('${uuidv4()}','${site}','${status}')`,
                (err, result) => {
                    if (err) {
    ...
    query: (search, status, sortBy, limit, offset) => {
        let orderQuery = `ORDER BY site ${sortBy} LIMIT ${limit} OFFSET ${offset}`
    ...
    readDetail: (id) => {
        return new Promise((resolve, reject) => {
            db.query(
                `SELECT * from seats_left WHERE id='${id}'`,
    ...
    update: ({ id, status }) => {
        return new Promise((resolve, reject) => {
            db.query(`SELECT * FROM seats_left WHERE id='${id}'`, (err, result) => {
                // console.log(result);
                if (err) {
                    return reject(err.message);
                } else {
                    db.query(
                        `UPDATE seats_left SET status='${status || result.rows[0].status}' WHERE id='${id}'`,
                        (err, result) => {
                            if (err) {
                                return reject(err.message)
                            } else {
    ...
    remove: (id) => {
        return new Promise((resolve, reject) => {
            db.query(
                `DELETE from seats_left WHERE id='${id}'`,
                (err, result) => {
                    if (err) {
                        return reject(err.message)
                    } else {
